...
const kpis = computed(() => {
  ...
  if (!s) return []
  const retentionColor = s.retentionRate >= 40 ? '#1B4332' : s.retentionRate >= 20 ? '#E8920A' : '#DC2626'
  return [
...
const donutColors = ['#1B4332', '#2D6A4F', '#52B788', '#E8920A', '#8B5CF6', '#0EA5E9', '#DC2626', '#F59E0B', '#A78BFA', '#06B6D4', '#6B7A72']
...
<template>
  <div style="display: flex; flex-direction: column; gap: 24px;">
    ...
    <div v-if="loading" class="kpi-grid">
      <div v-for="i in 6" :key="i" style="height: 100px; background: #fff; border-radius: 12px; border: 1px solid #ECEFEB; animation: pulse 1.5s ease-in-out infinite;" />
    </div>
    ...
    <div v-else-if="data" class="kpi-grid">
      <div
        v-for="kpi in kpis"
        :key="kpi.label"
        style="background: #fff; border: 1px solid #ECEFEB; border-radius: 14px; padding: 20px; display: flex; flex-direction: column; gap: 12px; cursor: default;"
        :title="kpi.tooltip ?? ''"
      >
        ...
        <div>
          <div style="font-size: 28px; font-weight: 800; letter-spacing: -0.03em; line-height: 1;" :style="{ color: kpi.valueColor ?? '#0F1B14' }">{{ kpi.value }}</div>
      ...
      <div style="background: #fff; border: 1px solid #ECEFEB; border-radius: 14px; padding: 20px; flex: 1; min-width: 0;">
        ...
        <div style="margin-bottom: 16px; display: flex; align-items: flex-start; justify-content: space-between; gap: 12px;">
          ...
          <div style="text-align: right; flex-shrink: 0; min-width: 72px;">
            <div style="font-size: 22px; font-weight: 800; letter-spacing: -0.03em; line-height: 1; color: #1B4332;">
              {{ hoveredBar ? hoveredBar.count : totalBarScans.toLocaleString() }}
            </div>
            <div style="font-size: 10.5px; color: #A8B3AC; margin-top: 3px;">
        ...
        <div v-if="barPoints.length" style="display: flex; gap: 0; align-items: stretch;">
          ...
          <div style="width: 34px; display: flex; flex-direction: column; justify-content: space-between; align-items: flex-end; padding-bottom: 20px; padding-right: 8px; flex-shrink: 0;">
            <span style="font-size: 9px; color: #A8B3AC; line-height: 1;">{{ barMax }}</span>
            <span style="font-size: 9px; color: #A8B3AC; line-height: 1;">{{ Math.round(barMax / 2) }}</span>
            <span style="font-size: 9px; color: #A8B3AC; line-height: 1;">0</span>
          </div>

          <div style="flex: 1; min-width: 0;">
            <!-- Área de barras con grid -->
            <div style="position: relative; height: 128px; border-bottom: 1px solid #ECEFEB; border-left: 1px solid #F3F5F2;">
              <div style="position: absolute; top: 0;   left: 0; right: 0; border-top: 1px dashed #F3F5F2;" />
              <div style="position: absolute; top: 50%; left: 0; right: 0; border-top: 1px dashed #F3F5F2;" />
              <!-- Barras -->
            ...
            <!-- Eje X — primer, cada 7 días, último -->
            <div style="position: relative; height: 20px; margin-top: 5px;">
              <template v-for="(point, i) in barPoints" :key="i">
                <span
                  ...
                  style="position: absolute; top: 0; font-size: 9.5px; color: #A8B3AC; white-space: nowrap; transition: color 0.1s;"
                  :style="[
                    i === 0                    ? { left: '0' } :
                    i === barPoints.length - 1 ? { right: '0' } :
                    { left: `${(i / (barPoints.length - 1)) * 100}%`, transform: 'translateX(-50%)' },
                    { color: hoveredBar?.date === point.date ? '#1B4332' : '#A8B3AC',
        ...
        <div v-else style="height: 148px; display: flex; align-items: center; justify-content: center; color: #D8DDD7; font-size: 13px;">Sin datos</div>
      </div>
        ...
        <div style="display: flex; flex-direction: column; overflow-y: auto; max-height: 260px;">
          <template v-if="data.recentActivity?.length">
            ...
            >
              ...
              <div style="flex: 1; min-width: 0;">
                <p style="font-size: 12.5px; color: #0F1B14; margin: 0; font-weight: 500; line-height: 1.4;">{{ activityDescription(evt) }}</p>
                <p style="font-size: 11px; color: #A8B3AC; margin: 2px 0 0;">{{ evt.walletName }}</p>
              </div>
              <span style="font-size: 10.5px; color: #A8B3AC; white-space: nowrap; flex-shrink: 0; margin-top: 2px;">{{ relativeTime(evt.createdAt) }}</span>
            </div>
          </template>
    ...
    <div v-if="data" style="background: #fff; border: 1px solid #ECEFEB; border-radius: 14px; padding: 24px;">
      <div style="margin-bottom: 20px;">
        <h3 style="font-size: 14px; font-weight: 700; color: #0F1B14; margin: 0;">Actividad por tipo de evento</h3>
      ...
      <div v-if="donutSegments.length" class="donut-layout">
        ...
        <div class="donut-legend">
          ...
          >
            ...
            <span style="flex: 1; font-size: 12px; color: #3A4A41; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ seg.label }}</span>
            <span style="font-size: 12px; font-weight: 700; color: #0F1B14; margin-left: 6px; flex-shrink: 0;">{{ seg.count.toLocaleString() }}</span>
            <span style="font-size: 11px; color: #6B7A72; min-width: 36px; text-align: right; flex-shrink: 0;">{{ seg.percent }}%</span>
          </div>
        </div>
      </div>

      <p v-else style="font-size: 13px; color: #A8B3AC; text-align: center; padding: 32px 0; margin: 0;">Sin actividad en este periodo</p>
    </div>
